...
//@ Let us for a moment reconsider the type `IntOrNothing`. Isn't it a bit annoying that we
...
//@ `SomethingOrNothing<i32>` to get back our `IntOrNothing`.
...
//@ `IntOrNothing` is just a type alias for `SomethingOrNothing<i32>`), and we can provide some
...
pub fn main() {
    let vec_int = vec![18, 5, 7, 3, 9, 27];
    let vec_float = vec![18.1, 5.2, 7.3, 3.4, 9.5, 27.6];
    let min1 = vec_min(vec_int);
    min1.print();
    let min2 = vec_min(vec_float);
    min2.print_float();
}
