...
pub trait ReadableNum: FromStr + std::fmt::Display {}
impl ReadableNum for i32{}
impl ReadableNum for f32{}
...
//@ ispecifying its type parameter doesn't tell us all that much.
impl<T: ReadableNum> SomethingOrNothing<T> {
pub fn read_vec() -> Vec<T> {
...
pub fn main() {
    let vec: Vec<f32> = SomethingOrNothing::read_vec();
    let min = vec_min(vec);                                         /*@*/
    min.print2();                                                    /*@*/
}
